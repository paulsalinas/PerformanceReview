...
export default class FeedbackRemoteContainer extends Component {
  ...
  componentDidMount() {
    ...
    const uri = `${REVIEW_URL}?${encodeURI("where=" + JSON.stringify({
          employeeId
        }))}`;

    return fetch(uri, options).then(response => response.json()).then(data =>
      this.setState({
          ...
          {reviews.map(review => (
            <ListGroupItem key={review.objectId}>
              <FeedbackForm
                review={review}
                onDone={feedback =>
                  this._updateFeedback(review.objectId, feedback)}
              />
            </ListGroupItem>
          ))}
        </ListGroup>;
